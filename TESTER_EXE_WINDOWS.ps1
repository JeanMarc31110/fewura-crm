...
try {
    ...
    for($i=0;$i -lt 80;$i++){Start-Sleep -Milliseconds 250;try{$h=Invoke-RestMethod "$base/health" -TimeoutSec 2;if($h.ok -and $h.version -eq '1.4.4' -and $h.scheduler){$ok=$true;break}}catch{}}
    if(-not $ok){throw 'Health 1.4.4 inaccessible.'}
    ...
    Write-Host 'FEWURA CRM 1.4.4 INTERFACE + LANCEUR + CRM + PROSPECT + EMAIL + SMS OK' -ForegroundColor Green
    exit 0
}catch{
