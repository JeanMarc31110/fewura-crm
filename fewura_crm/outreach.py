...
def whatsapp_status() -> dict:
    return {
        "configured": bool(get_setting("whatsapp_token") and get_setting("whatsapp_phone_number_id")),
...
def save_smtp(host: str, port: int, username: str, password: str, from_email: str, from_name: str, security: str) -> None:
    ...
    values = {
        "smtp_host": host.strip(), "smtp_port": str(int(port)), "smtp_username": username.strip(),
        "smtp_from_email": from_email.strip(), "smtp_from_name": from_name.strip() or "FEWURA CRM",
...
def save_whatsapp(phone_number_id: str, token: str) -> None:
    ...
    if token:
        values["whatsapp_token"] = token.strip()
    set_settings(values)
...
def _send_whatsapp(prospect: dict, body: str) -> None:
    cfg = whatsapp_status()
    if not cfg["configured"]:
        raise RuntimeError("WhatsApp Cloud non configuré")
...
def run_campaign(campaign_id: int, force_mode: str | None = None, max_items: int = 500) -> dict:
    ...
    if not camp:
        con.close(); raise ValueError("Campagne introuvable")
    camp = dict(camp); mode = force_mode or camp["mode"]
    if mode not in {"simulation", "reel"}:
        con.close(); raise ValueError("Mode invalide")
    ...
    for row in recips:
        ...
        try:
            ...
            if mode == "simulation":
                status = "simulated"; stats["simulated"] += 1
            else:
                if channel == "email": _send_email(p, subject, body)
                else: _send_whatsapp(p, body)
                status = "sent"; stats["sent"] += 1
            stats[channel] += 1
            con.execute("UPDATE campaign_recipients SET status=?,attempts=attempts+1,last_error=NULL,sent_at=CASE WHEN ?='sent' THEN CURRENT_TIMESTAMP ELSE sent_at END WHERE id=?", (status,status,rid))
