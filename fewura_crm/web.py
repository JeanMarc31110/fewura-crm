...
def dashboard():
    ...
    body+='<section class="card"><h2>État des canaux</h2>'+('<p><span class="pill ok">SMTP configuré</span></p>' if smtp_status()['configured'] else '<p><span class="pill bad">SMTP à configurer</span></p>')+('<p><span class="pill ok">SMS téléphone configuré — '+esc(sms['sender_number'])+'</span></p>' if sms['configured'] else '<p><span class="pill warnpill">SMS téléphone à connecter — +33 7 73 54 78 57</span></p>')+'<p><a class="btn" href="/settings">Configurer les envois</a></p></section></div>'
    return layout(body)
