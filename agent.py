...
def self_test() -> int:
    init_db()
    checks = {"database": data_dir().exists(), "web_interface": app is not None, "local_only": HOST == "127.0.0.1", "version": VERSION == "1.3.0"}
...
def _open_when_ready() -> None:
    ...
    for _ in range(120):
        if _port_open(): webbrowser.open(URL); return
        time.sleep(0.25)
...
def main() -> int:
    ...
    def watch_shutdown():
        while not server.should_exit:
            if shutdown_requested(): server.should_exit=True; return
            time.sleep(0.2)
